...
- 若要自行架站，可改用交付的 `ANG-HR-BETA-v0.2-deploy.zip`。
- 畫面必須顯示 `BETA-v0.2`；若顯示 `BETA-v0.1`，請停止測試並回報網址與截圖。
- 原本的 `BETA-v0.1` 是固定舊版，仍供舊回報對版，不會被 v0.2 覆蓋。

## 臨時測試帳號

以下全部都是固定假資料，只能用於 `BETA-v0.2`。本版入口接受 7 組「一帳號一方案」臨時資料。企業與個人方案共用臨時密碼 `AngBeta#2026`，入口會核對「帳號＋Email＋臨時密碼」；免費方案使用固定驗證碼 `123456`。
